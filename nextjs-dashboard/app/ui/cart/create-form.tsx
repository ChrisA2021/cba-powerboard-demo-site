...
import CreditCardWidget from '@/app/ui/cart/credit-card-widget';
import PayPalWidget from '@/app/ui/cart/paypal-widget';
import AfterPayWidget from '@/app/ui/cart/afterpay-widget';
import createWalletTokenRequest from '@/app/api/wallet-token'
...
export default function Form() {
  ...
  const handlePaymentMethodChange = (event: { target: { value: any; }; }) => {
    ...
    if (newValue === PaymentMethods.PAYPAL) {
      createWalletToken();
    }
  };
  ...
  const createWalletToken = async () => {
    try {
      const apiResponse = await createWalletTokenRequest(
        "1.00",
        '123456',
        'John',
        'Doe',
        'john.doe@example.com',
        process.env.NEXT_PUBLIC_POWERBOARD_GATEWAY_ID,
        process.env.NEXT_PUBLIC_POWERBOARD_SECRET_KEY
      );
      setWalletToken(apiResponse);
    } catch (error) {
      // Handle errors
      console.error('Error handling charge submission:', error);
    }
    return walletToken
  };
      ...
      <div className="rounded-md bg-gray-50 p-4 md:p-6">
        ...
        {selectedPaymentMethod === PaymentMethods.CREDIT_CARD && (
          <div className="mb-4">
            ...
            <CreditCardWidget publicKey={process.env.NEXT_PUBLIC_POWERBOARD_PUBLIC_KEY} gatewayId={process.env.NEXT_PUBLIC_POWERBOARD_GATEWAY_ID} />
          </div>
        )}
        {/* PayPal Widget */}
        {selectedPaymentMethod === PaymentMethods.PAYPAL && (
          <div className="mb-4">
            <label htmlFor="payPalWidget" className="mb-2 block text-sm font-medium">
              PayPal
            </label>
            <PayPalWidget walletToken={walletToken} />
          </div>
        )}
        {/* AfterPay Widget */}
        {selectedPaymentMethod === PaymentMethods.AFTERPAY && (
          <div className="mb-4">
            <label htmlFor="afterPayWidget" className="mb-2 block text-sm font-medium">
              AfterPay
            </label>
            <AfterPayWidget walletToken={walletToken} />
          </div>
        )}
        {/* Form Error */}
